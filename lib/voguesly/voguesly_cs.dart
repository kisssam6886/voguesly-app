...
/// Windows/Linux 需改行外部浏览器:webview_flutter **冇桌面(Win/Linux)实现**
/// (windows generated_plugins 只有 desktop_webview_window + url_launcher_windows,
/// 冇 webview_flutter_windows),内嵌 WebViewController() 会喺运行时抛异常 → Sam 实测
/// Windows「在线客服」打唔开(同 Google 登录闪退同源)。macOS 有 WKWebView 实现,保留内嵌。
...
Future<void> _launchCsExternal(ProviderContainer container) async {
  try {
    ...
    if (!ok) {
      globalState.showNotifier('打不开浏览器,请手动访问客服页');
    }
  } catch (_) {
...
class VogueslyCsPanel extends ConsumerStatefulWidget {
  ...
  /// 开客服。桌面 Win/Linux → 系统浏览器(webview 冇实现,内嵌会崩);
